use pilota::thrift::{ApplicationError, EntryMessage, Message};
...
impl<M> ThriftMessage<M> {
    #[inline]
    pub fn mk_client_msg(cx: &ClientContext, msg: Result<M, Error>) -> Result<Self, crate::Error> {
    ...
    #[inline]
    pub fn mk_server_resp(cx: &ServerContext, msg: Result<M, Error>) -> Result<Self, crate::Error> {
        let meta = MessageMeta {
            msg_type: match msg {
...
where
    U: EntryMessage + Send,
{
    pub(crate) fn encode<T: TOutputProtocol>(&self, protocol: &mut T) -> Result<(), Error> {
        let ident = TMessageIdentifier::new(
            self.meta.method.clone(),
            self.meta.msg_type,
            self.meta.seq_id,
            ...
            Err(e) => match e {
                ...
                Error::Protocol(e) => {
                    ...
                    let e = ApplicationError::new(
                        pilota::thrift::ApplicationErrorKind::ProtocolError,
                        e.message.clone(),
                    );
                    e.encode(protocol)?;
                }
    ...
    pub(crate) fn decode<Cx: ThriftContext, T: TInputProtocol>(
        ...
        let res = match msg_ident.message_type {
            TMessageType::Exception => Err(Error::Application(Message::decode(protocol)?)),
        ...
        R: AsyncRead + Unpin + Send,
    {
        ...
        let res = match msg_ident.message_type {
            TMessageType::Exception => {
                Err(Error::Application(Message::decode_async(protocol).await?))
            }
