...
use volo_gen::proto_gen::google::cloud::beyondcorp::appgateways::v1::{AppGatewaysServiceClient, AppGatewaysServiceClientBuilder};
...
use crate::discover::LazyDiscover;
...
macro_rules! service_impl {
    ($builder_ty:ty, $self:expr, $endpoint:expr) => {{
        let mut builder = <$builder_ty>::new($self.callee_name.to_string())
            ...
            .layer_inner_front(
                HeaderLayer::new($endpoint.clone())
