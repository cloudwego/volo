...
async fn main() -> io::Result<()> {
    ...
    println!(
        "named pipe full-duplex stress test passed: {} connections, {} messages each side, {} \
         bytes each message, elapsed {:?}",
        cfg.connections,
        cfg.iterations,
        cfg.payload_len,
        started_at.elapsed(),
...
async fn run_client_pool(cfg: Arc<StressConfig>) -> io::Result<()> {
    ...
    for conn_id in 0..cfg.connections {
        let cfg = cfg.clone();
        tasks.push(tokio::spawn(async move {
            run_client_connection(cfg, conn_id).await
        }));
    }
...
async fn run_duplex(
    ...
    let reader =
        tokio::spawn(async move { read_frames(&mut read_half, &cfg, conn_id, read_marker).await });
    ...
    for seq in 0..cfg.iterations {
        ...
        if len != cfg.payload_len {
            return Err(io::Error::new(
                ...
                format!(
                    "unexpected payload length on connection {expected_conn_id}: expected {}, got \
                     {len}",
                    cfg.payload_len
                ),
            ));
...
fn validate_payload(
    ...
    if seq != expected_seq {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            format!(
                "unexpected sequence on connection {expected_conn_id}: expected {expected_seq}, \
                 got {seq}"
            ),
        ));
    }
