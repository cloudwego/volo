//! These codes are copied from `tonic/src/metadata/map.rs` and may be modified by us.

use std::marker::PhantomData;

pub(crate) use self::{
    as_encoding_agnostic_metadata_key::AsEncodingAgnosticMetadataKey,
    as_metadata_key::AsMetadataKey, into_metadata_key::IntoMetadataKey,
};
use super::{
    encoding::{Ascii, Binary, ValueEncoding},
...
impl MetadataMap {
    ...
    /// let mut map = MetadataMap::default();
    /// map.insert_bin("trace-proto-bin", MetadataValue::from_bytes(b"hello"));
    /// map.get_bin_mut("trace-proto-bin")
    ///     .unwrap()
    ///     .set_sensitive(true);
    ///
    ...
    ///     match key_and_value {
    ///         KeyAndValueRef::Ascii(ref key, ref value) => println!("Ascii: {:?}: {:?}", key, value),
    ...
    ///     match key_and_value {
    ///         KeyAndMutValueRef::Ascii(key, mut value) => value.set_sensitive(true),
    ///         KeyAndMutValueRef::Binary(key, mut value) => value.set_sensitive(false),
    ///     }
    /// }
    /// ```
    ...
    /// for key in map.keys() {
    ///     match key {
    ///         KeyRef::Ascii(ref key) => println!("Ascii key: {:?}", key),
    ///         KeyRef::Binary(ref key) => println!("Binary key: {:?}", key),
    ///     }
    ///     println!("{:?}", key);
    /// }
    /// ```
    ...
    ///     match value {
    ///         ValueRef::Ascii(ref value) => println!("Ascii value: {:?}", value),
    ///         ValueRef::Binary(ref value) => println!("Binary value: {:?}", value),
    ///     }
    ///     println!("{:?}", value);
    /// }
    /// ```
    ...
    ///     match value {
    ///         ValueRefMut::Ascii(mut value) => value.set_sensitive(true),
    ///         ValueRefMut::Binary(mut value) => value.set_sensitive(false),
    ///     }
    /// }
    /// ```
    ...
    ///     *counter = format!("{}{}", counter.to_str().unwrap(), "1")
    ///         .parse()
    ///         .unwrap();
    /// }
    ///
    ...
    ///         .entry_bin(header)
    ///         .unwrap()
    ///         .or_insert(MetadataValue::from_bytes(b""));
    ///     *counter = MetadataValue::from_bytes(
    ///         format!(
    ///             "{}{}",
    ///             str::from_utf8(counter.to_bytes().unwrap().as_ref()).unwrap(),
    ///             "1"
    ///         )
    ///         .as_bytes(),
    ///     );
    ...
    ///
    /// let mut prev = map
    ///     .insert_bin("trace-proto-bin", MetadataValue::from_bytes(b"earth"))
    ///     .unwrap();
    /// assert_eq!("world", prev);
    /// ```
    ///
...
impl<'a, VE: ValueEncoding> Entry<'a, VE> {
    ...
    ///
    /// let keys = &["content-length", "x-hello", "Content-Length", "x-world"];
    ///
    /// for &key in keys {
    ///     let counter = map
    ///         .entry(key)
...
mod tests {
    ...
    fn test_iter_mut_categorizes_ascii_entries() {
        ...
        let mut found_x_word = false;
        for key_and_value in map.iter_mut() {
            if let KeyAndMutValueRef::Ascii(key, ref _value) = key_and_value {
                if key.as_str() == "x-word" {
                    found_x_word = true;
                } else {
                    panic!("Unexpected key");
    ...
    fn test_iter_mut_categorizes_binary_entries() {
        ...
        let mut found_x_word_bin = false;
        for key_and_value in map.iter_mut() {
            if let KeyAndMutValueRef::Binary(key, ref _value) = key_and_value {
                if key.as_str() == "x-word-bin" {
                    found_x_word_bin = true;
                } else {
                    panic!("Unexpected key");
